...
class CiliaDatasetPred(Dataset):
    def __init__(self, cdata, target, metadata, n_sequence=5):
        # n_sequence > 0
        ...
        self.time      = torch.split(self.metadata[0],self.n_sequence)  # time
        self.xloc      = torch.split(self.metadata[1],self.n_sequence)  # xloc
        self.yloc      = torch.split(self.metadata[2],self.n_sequence)  # yloc
        self.theta      = torch.split(self.metadata[3],self.n_sequence) # theta
        self.a      = torch.split(self.metadata[4],self.n_sequence) # theta
...
def predict(data_loader, model):
    ...
    model.eval()
    with torch.no_grad():
        for X, y, metadata in data_loader:
            y_star = model(X)
            ...
            tsec    = torch.cat((tsec, metadata[0]),0)
            xloc    = torch.cat((xloc, metadata[1]),0)
            yloc    = torch.cat((yloc, metadata[2]),0)
            thetap  = torch.cat((thetap, metadata[3]),0)
            a       = torch.cat((a, metadata[4]),0)
    return output, actual, (tsec,xloc,yloc,thetap,a) 
